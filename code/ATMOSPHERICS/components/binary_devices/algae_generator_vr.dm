...
#define MATERIAL_CARBON "graphite"
...
/obj/machinery/atmospherics/binary/algae_farm/process()
	...
	if(stored_material[MATERIAL_ALGAE] < algae_per_mole)
		...
		return
	if(stored_material[MATERIAL_CARBON] + graphite_per_mole > storage_capacity[MATERIAL_CARBON])
	...
	var/moles_to_convert = min(moles_per_tick,\
		...
		storage_capacity[MATERIAL_CARBON] - stored_material[MATERIAL_CARBON] * graphite_per_mole)
	...
	stored_material[MATERIAL_CARBON] += converted_moles * graphite_per_mole
